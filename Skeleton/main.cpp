...
int main()
{
  ...
  bind.bindNew("amplitude", amplitude, RhIO::Bind::PullOnly)
      ...
      ->minimum(0)
      ->maximum(15);
  bind.bindNew("freq", freq, RhIO::Bind::PullOnly)->defaultValue(1.0)->persisted(true)->minimum(0);

  // Adding a "status" command that will be available in the shell
  bind.node().newCommand("status", "Demo status command", [&amplitude, &freq](std::vector<std::string>) -> std::string {
    std::stringstream ss;
