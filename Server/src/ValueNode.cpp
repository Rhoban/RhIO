...
ValueType ValueNode::getValueType(const std::string& name) const
{
  try
  {
    ...
    if (child != nullptr)
      return child->getValueType(tmpName);
  }
...
bool ValueNode::getBool(const std::string& name) const
{
  ...
  ValueNode* child = BaseNode::forwardFunc(name, tmpName, false);
  if (child != nullptr)
    return child->getBool(tmpName);

  std::lock_guard<std::mutex> lock(_mutex);
  if (_valuesBool.count(name) == 0)
  {
    throw std::logic_error("RhIO unknown value Bool name: '" + name + "' in '" + BaseNode::pwd + "'");
  }
  else
  {
...
int64_t ValueNode::getInt(const std::string& name) const
{
  ...
  ValueNode* child = BaseNode::forwardFunc(name, tmpName, false);
  if (child != nullptr)
    return child->getInt(tmpName);

  std::lock_guard<std::mutex> lock(_mutex);
  if (_valuesInt.count(name) == 0)
  {
    throw std::logic_error("RhIO unknown Int value name: '" + name + "' in '" + BaseNode::pwd + "'");
  }
  else
  {
...
double ValueNode::getFloat(const std::string& name) const
{
  ...
  ValueNode* child = BaseNode::forwardFunc(name, tmpName, false);
  if (child != nullptr)
    return child->getFloat(tmpName);

  std::lock_guard<std::mutex> lock(_mutex);
  if (_valuesFloat.count(name) == 0)
  {
    throw std::logic_error("RhIO unknown Float value name: '" + name + "' in '" + BaseNode::pwd + "'");
  }
  else
  {
...
const std::string& ValueNode::getStr(const std::string& name) const
{
  ...
  ValueNode* child = BaseNode::forwardFunc(name, tmpName, false);
  if (child != nullptr)
    return child->getStr(tmpName);

  std::lock_guard<std::mutex> lock(_mutex);
  if (_valuesStr.count(name) == 0)
  {
    throw std::logic_error("RhIO unknown Str value name: '" + name + "' in '" + BaseNode::pwd + "'");
  }
  else
  {
...
void ValueNode::setBool(const std::string& name, bool val, bool noCallblack,
                        std::chrono::steady_clock::time_point timestamp)
{
  ...
  {
    // Bound to min/max
    if (_valuesBool.at(name).hasMin && val < _valuesBool.at(name).min)
    {
      val = _valuesBool.at(name).min;
    }
    if (_valuesBool.at(name).hasMax && val > _valuesBool.at(name).max)
    {
    ...
    if (_valuesBool.at(name).streamWatchers > 0)
    {
      ServerStream->publishBool(
          BaseNode::pwd + separator + name, val,
          std::chrono::duration_cast<std::chrono::milliseconds>(timestamp.time_since_epoch()).count());
    }
  }
}
void ValueNode::setInt(const std::string& name, int64_t val, bool noCallblack,
                       std::chrono::steady_clock::time_point timestamp)
{
  // Forward to subtree
  std::string tmpName;
  ...
  if (_valuesInt.count(name) == 0)
  {
    throw std::logic_error("RhIO unknown value Int name: '" + name + "' in '" + BaseNode::pwd + "'");
  }
  else
  {
    // Bound to min/max
    if (_valuesInt.at(name).hasMin && val < _valuesInt.at(name).min)
    {
      val = _valuesInt.at(name).min;
    }
    if (_valuesInt.at(name).hasMax && val > _valuesInt.at(name).max)
    {
...
void ValueNode::setFloat(const std::string& name, double val, bool noCallblack,
                         std::chrono::steady_clock::time_point timestamp)
{
  // Forward to subtree
  std::string tmpName;
  ...
  if (_valuesFloat.count(name) == 0)
  {
    throw std::logic_error("RhIO unknown value Float name: '" + name + "' in '" + BaseNode::pwd + "'");
  }
  else
  {
    // Bound to min/max
    if (_valuesFloat.at(name).hasMin && val < _valuesFloat.at(name).min)
    {
      val = _valuesFloat.at(name).min;
    }
    if (_valuesFloat.at(name).hasMax && val > _valuesFloat.at(name).max)
    {
...
void ValueNode::setStr(const std::string& name, const std::string& val, bool noCallblack,
                       std::chrono::steady_clock::time_point timestamp)
{
  // Forward to subtree
  std::string tmpName;
  ...
  if (_valuesStr.count(name) == 0)
  {
    throw std::logic_error("RhIO unknown value Str name: '" + name + "' in '" + BaseNode::pwd + "'");
  }
  else
  {
    // Update value
    ...
    // Bound to min/max
    if (_valuesStr.at(name).hasMin && val < _valuesStr.at(name).min)
    {
      _valuesStr[name].value = _valuesStr.at(name).min;
    }
    if (_valuesStr.at(name).hasMax && val > _valuesStr.at(name).max)
    {
...
std::unique_ptr<ValueBuilderBool> ValueNode::newBool(const std::string& name)
{
  ...
  if (getValueType(name) != NoValue)
  {
    ...
    if (getValueType(name) != TypeBool)
    {
      ...
      throw std::logic_error("RhIO value already known with other type: '" + name + "' in '" + BaseNode::pwd + "'");
    }
    else
    {
      // No conflic
      return std::unique_ptr<ValueBuilderBool>(new ValueBuilderBool(_valuesBool[name], true));
    }
  }
  else
  {
    ...
    return std::unique_ptr<ValueBuilderBool>(new ValueBuilderBool(_valuesBool[name], false));
  }
}
std::unique_ptr<ValueBuilderInt> ValueNode::newInt(const std::string& name)
{
  ...
  if (getValueType(name) != NoValue)
  {
    ...
    {
      // No conflic
      return std::unique_ptr<ValueBuilderInt>(new ValueBuilderInt(_valuesInt[name], true));
    }
  }
  else
  {
    ...
    return std::unique_ptr<ValueBuilderInt>(new ValueBuilderInt(_valuesInt[name], false));
  }
}
std::unique_ptr<ValueBuilderFloat> ValueNode::newFloat(const std::string& name)
{
  ...
  if (getValueType(name) != NoValue)
  {
    ...
    {
      // No conflic
      return std::unique_ptr<ValueBuilderFloat>(new ValueBuilderFloat(_valuesFloat[name], true));
    }
  }
  else
  {
    ...
    return std::unique_ptr<ValueBuilderFloat>(new ValueBuilderFloat(_valuesFloat[name], false));
  }
}
std::unique_ptr<ValueBuilderStr> ValueNode::newStr(const std::string& name)
{
  ...
  if (getValueType(name) != NoValue)
  {
    ...
    {
      // No conflic
      return std::unique_ptr<ValueBuilderStr>(new ValueBuilderStr(_valuesStr[name], true));
    }
  }
  else
  {
    ...
    return std::unique_ptr<ValueBuilderStr>(new ValueBuilderStr(_valuesStr[name], false));
  }
}

void ValueNode::setCallbackBool(const std::string& name, std::function<void(bool)> func)
{
  // Forward to subtree
  std::string tmpName;
  ValueNode* child = BaseNode::forwardFunc(name, tmpName, false);
  if (child != nullptr)
    return child->setCallbackBool(tmpName, func);
...
void ValueNode::setCallbackInt(const std::string& name, std::function<void(int64_t)> func)
{
  // Forward to subtree
  std::string tmpName;
  ValueNode* child = BaseNode::forwardFunc(name, tmpName, false);
  if (child != nullptr)
    return child->setCallbackInt(tmpName, func);
...
void ValueNode::setCallbackFloat(const std::string& name, std::function<void(double)> func)
{
  // Forward to subtree
  std::string tmpName;
  ValueNode* child = BaseNode::forwardFunc(name, tmpName, false);
  if (child != nullptr)
    return child->setCallbackFloat(tmpName, func);
...
void ValueNode::setCallbackStr(const std::string& name, std::function<void(std::string)> func)
{
  // Forward to subtree
  std::string tmpName;
  ValueNode* child = BaseNode::forwardFunc(name, tmpName, false);
  if (child != nullptr)
    return child->setCallbackStr(tmpName, func);
...
const ValueBool& ValueNode::getValueBool(const std::string& name) const
{
  ...
  ValueNode* child = BaseNode::forwardFunc(name, tmpName, false);
  if (child != nullptr)
    return child->getValueBool(tmpName);
...
const ValueInt& ValueNode::getValueInt(const std::string& name) const
{
  ...
  ValueNode* child = BaseNode::forwardFunc(name, tmpName, false);
  if (child != nullptr)
    return child->getValueInt(tmpName);
...
const ValueFloat& ValueNode::getValueFloat(const std::string& name) const
{
  ...
  ValueNode* child = BaseNode::forwardFunc(name, tmpName, false);
  if (child != nullptr)
    return child->getValueFloat(tmpName);
...
const ValueStr& ValueNode::getValueStr(const std::string& name) const
{
  ...
  ValueNode* child = BaseNode::forwardFunc(name, tmpName, false);
  if (child != nullptr)
    return child->getValueStr(tmpName);
...
void ValueNode::enableStreamingValue(const std::string& name)
{
  ...
  {
    throw std::logic_error("RhIO unknown value name: '" + name + "' in '" + BaseNode::pwd + "'");
  }
}
...
void ValueNode::saveValues(const std::string& path)
{
  ...
  if (!file.is_open())
  {
    throw std::runtime_error("RhIO unable to write values file: " + path);
  }
  ...
  for (auto& v : _valuesBool)
  {
    if (v.second.persisted)
    {
      file << "[bool] " << v.second.name << ".value = " << v.second.value << std::endl;
      file << "[bool] " << v.second.name << ".comment = " << v.second.comment << std::endl;
  ...
  for (auto& v : _valuesInt)
  {
    if (v.second.persisted)
    {
      file << "[int] " << v.second.name << ".value = " << v.second.value << std::endl;
      file << "[int] " << v.second.name << ".comment = " << v.second.comment << std::endl;
      // Update persisted value
      v.second.valuePersisted = v.second.value;
    }
  }
  ...
  for (auto& v : _valuesFloat)
  {
    if (v.second.persisted)
    {
      file << "[float] " << v.second.name << ".value = " << v.second.value << std::endl;
      file << "[float] " << v.second.name << ".comment = " << v.second.comment << std::endl;
      // Update persisted value
      v.second.valuePersisted = v.second.value;
    }
  }
  ...
  for (auto& v : _valuesStr)
  {
    if (v.second.persisted)
    {
      file << "[str] " << v.second.name << ".value = " << v.second.value << std::endl;
      file << "[str] " << v.second.name << ".comment = " << v.second.comment << std::endl;
      // Update persisted value
      v.second.valuePersisted = v.second.value;
    }
  }
...
void ValueNode::loadValues(const std::string& path)
{
  ...
  auto error = [&path](const std::string& line) {
    throw std::runtime_error("RhIO invalid formated values file: " + path + ": " + line);
  };
