...
bool StreamNode::streamExist(const std::string& name) const
{
  ...
  if (child != nullptr)
    return child->streamExist(tmpName);

  std::lock_guard<std::mutex> lock(_mutex);
  return (_streams.count(name) > 0);
}

std::string StreamNode::streamDescription(const std::string& name) const
{
  // Forward to subtree
  std::string tmpName;
  StreamNode* child = BaseNode::forwardFunc(name, tmpName, false);
  if (child != nullptr)
    return child->streamDescription(tmpName);
  ...
  {
    throw std::logic_error("RhIO unknown stream name: " + name);
  }
}
...
void StreamNode::newStream(const std::string& name, const std::string& comment)
{
  // Forward to subtree
  std::string tmpName;
  ...
  if (_streams.count(name) == 0)
  {
    ...
    _streams.at(name).buffer = std::make_shared<StreamBuffer>(BaseNode::pwd + separator + name);
    _streams.at(name).stream = std::make_shared<std::ostream>(_streams.at(name).buffer.get());
  }
  else
  {
    throw std::logic_error("RhIO already register stream name: " + name);
  }
}
