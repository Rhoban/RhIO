...
public:
   ...
   */
  typedef std::function<std::string(std::vector<std::string>)> CommandFunc;
   ...
   * exist
   */
  std::string commandDescription(const std::string& name) const;
   ...
   * (mutex deadlock).
   */
  void newCommand(const std::string& name, const std::string& comment, CommandFunc func);
