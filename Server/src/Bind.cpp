...
namespace RhIO
{
Bind::Bind(const std::string& prefixChild) : _prefix(prefixChild)
{
...
std::unique_ptr<ValueBuilderBool> Bind::bindNew(const std::string& name, bool& var, Policy policy)
{
  // Check that variable is not already registered
  if (checkIsRegistered(&var))
  {
    throw std::logic_error("RhIO::Bind: Variable address already bind: " + name);
  }
  // Create non existing hierarchy
...
void Bind::bind(const std::string& name, bool& var, Policy policy)
{
  ...
  {
    throw std::logic_error("RhIO not created value name: " + name);
  }
  // Add to bind container
  _bindsBool.push_back({ relativeName(_prefix + name), getChildPtr(_prefix + name), &var, policy });
}
void Bind::bind(const std::string& name, int& var, Policy policy)
{
  // Check that variable is not already registered
  if (checkIsRegistered(&var))
  {
...
void Bind::bind(const std::string& name, long& var, Policy policy)
{
  // Check that variable is not already registered
  if (checkIsRegistered(&var))
  {
...
void Bind::bind(const std::string& name, float& var, Policy policy)
{
  // Check that variable is not already registered
  if (checkIsRegistered(&var))
  {
...
void Bind::bind(const std::string& name, double& var, Policy policy)
{
  // Check that variable is not already registered
  if (checkIsRegistered(&var))
  {
...
void Bind::bind(const std::string& name, std::string& var, Policy policy)
{
  // Check that variable is not already registered
  if (checkIsRegistered(&var))
  {
...
void Bind::newStream(const std::string& name, const std::string& comment)
{
  // Create non existing hierarchy
  createPath(_prefix + name);
...
void Bind::newFrame(const std::string& name, const std::string& comment, FrameFormat format)
{
  // Create non existing hierarchy
  createPath(_prefix + name);
...
void Bind::framePush(const std::string& name, size_t width, size_t height, unsigned char* data, size_t size)
{
