...
IONode::IONode()
  : ValueNode(
        std::bind(&IONode::forwardChildren, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3))
  , CommandNode(
        std::bind(&IONode::forwardChildren, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3))
  , StreamNode(
        std::bind(&IONode::forwardChildren, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3))
...
bool IONode::childExist(const std::string& name) const
{
  try
  {
    // Forward to subtree
    std::string tmpName;
    const IONode* child = const_cast<IONode*>(this)->forwardChildren(name, tmpName, false);
    if (child != nullptr)
      return child->childExist(tmpName);
  }
...
const IONode& IONode::child(const std::string& name) const
{
  ...
  const IONode* child = const_cast<IONode*>(this)->forwardChildren(name, tmpName, false);
  if (child != nullptr)
    return child->child(tmpName);

  std::lock_guard<std::mutex> lock(_mutex);
  if (_children.count(name) == 0)
  {
    throw std::logic_error("RhIO unknown node name: '" + name + "' in '" + pwd() + "'");
  }
  else
  {
...
void IONode::save(const std::string& path)
{
  ...
  for (size_t i = 0; i < list.size(); i++)
  {
    if (list[i] != "." && list[i] != "..")
    {
      ...
      {
        throw std::runtime_error("RhIO unexpected folder in save path: " + path + ": " + list[i]);
      }
    }
  }
...
void IONode::load(const std::string& path)
{
  ...
  for (size_t i = 0; i < list.size(); i++)
  {
    if (list[i] != "." && list[i] != ".." && _children.count(list[i]) == 0)
    {
...
IONode& IONode::operator=(const IONode& node)
{
  ...
  _parent = node._parent;
  _children = node._children;
  BaseNode<ValueNode>::forwardFunc =
      std::bind(&IONode::forwardChildren, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
  BaseNode<CommandNode>::forwardFunc =
      std::bind(&IONode::forwardChildren, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
  BaseNode<StreamNode>::forwardFunc =
      std::bind(&IONode::forwardChildren, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
  BaseNode<FrameNode>::forwardFunc =
      std::bind(&IONode::forwardChildren, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
  BaseNode<ValueNode>::pwd = _pwd;
  BaseNode<CommandNode>::pwd = _pwd;
  BaseNode<StreamNode>::pwd = _pwd;
...
IONode* IONode::forwardChildren(const std::string& name, std::string& newName, bool createBranch)
{
  IONode* pt = this;
  ...
  while (true)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        if (pt->_children.count(part) == 0)
        {
          throw std::logic_error("RhIO unknown node name: '" + part + "' in '" + pwd() + "'");
        }
      }
      std::lock_guard<std::mutex> lock(_mutex);
