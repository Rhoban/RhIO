...
bool CommandNode::commandExist(const std::string& name) const
{
  ...
  if (child != nullptr)
    return child->commandExist(tmpName);

  std::lock_guard<std::mutex> lock(_mutex);
  return (_commands.count(name) > 0);
}

std::string CommandNode::call(const std::string& name, const std::vector<std::string>& arguments)
{
  // Forward to subtree
  std::string tmpName;
  CommandNode* child = BaseNode::forwardFunc(name, tmpName, false);
  if (child != nullptr)
    return child->call(tmpName, arguments);
  ...
  {
    throw std::logic_error("RhIO unknown command name: " + name);
  }
}

std::string CommandNode::commandDescription(const std::string& name) const
{
  // Forward to subtree
  std::string tmpName;
  CommandNode* child = BaseNode::forwardFunc(name, tmpName, false);
  if (child != nullptr)
    return child->commandDescription(tmpName);
...
void CommandNode::newCommand(const std::string& name, const std::string& comment, CommandFunc func)
{
  // Forward to subtree
  std::string tmpName;
  ...
  {
    throw std::logic_error("RhIO already register command name: " + name);
  }
}
