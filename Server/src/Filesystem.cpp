...
std::vector<std::string> listDirectories(const std::string& path)
{
  ...
  if (dir == nullptr)
  {
    throw std::runtime_error("RhIO unable to open path directory: " + path);
  }
...
{
  std::string newPath;
  if (name.length() > 0 && name[0] != separator && path.length() > 0 && path[path.length() - 1] != separator)
  {
  ...
  for (; index >= 0; index--)
  {
    ...
    if (result != 0)
    {
      throw std::runtime_error("RhIO unable to create directory in: " + path);
    }
  }
}
