...
{
public:
  Test() : _bind("test"), _bind2("test5"), _bind3()
  {
    _bind.bindNew("valueBool", _valueBool)->comment("bool value")->persisted(true);
    _bind.bind("valueInt1", _valueInt);
    _bind.bind("test2/valueFloat1", _valueFloat);
    _bind.bind("valueInt2", _valueInt2, RhIO::Bind::PullOnly);
    _bind.bindNew("test2/test4/valueFloat2", _valueFloat2, RhIO::Bind::PushOnly)->minimum(-100.0);
    _valueFloat2 = -2.0;
    ...
    assert(RhIO::Root.call("test5/command1", {}).find("USAGE: command1 <int> <unsigned int|42> --> <int>") !=
           std::string::npos);
    ...
    assert(
        RhIO::Root.call("test/command3", { "3.0", "2" }).find("USAGE: command3 <float> <long> <string> --> <float>") !=
        std::string::npos);
