...
int main()
{
  RhIO::ClientReq client(std::string("tcp://localhost:") + RhIO::ServerRepPort);
  ...
  assert(client.metaValueFrame("test/frame1").format == RhIO::FrameFormat::RGB);
  assert(client.metaValueFrame("test/frame2").comment == "frame2");
  assert(client.metaValueFrame("test/frame2").format == RhIO::FrameFormat::BGR);
  ...
  assert(client.metaValueBool("test/paramBool").streamWatchers == 0);
  assert(client.metaValueInt("test/test3/paramInt").streamWatchers == 0);
  client.enableStreamingValue("test/test3/paramInt");
  assert(client.metaValueBool("test/paramBool").streamWatchers == 0);
  assert(client.metaValueInt("test/test3/paramInt").streamWatchers == 1);
