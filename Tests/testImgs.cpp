...
int main()
{
  RhIO::Root.newFrame("test/frame1", "Some frame stream", RhIO::FrameFormat::RGB);
  RhIO::IONode& node = RhIO::Root.child("test");
  node.newFrame("frame2", "Some frame stream", RhIO::FrameFormat::RGB);
  ...
  while (true)
  {
    ...
    {
      RhIO::Root.framePush("test/frame1", width1, height1, data1, size1);
    }
    if (RhIO::Root.frameIsStreaming("test/frame2"))
    {
      RhIO::Root.framePush("test/frame2", width2, height2, data2, size2);
    }
    line++;
    if (line > 100)
    {
