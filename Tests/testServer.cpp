...
int main()
{
  ...
  RhIO::Root.newFloat("test/sin")->minimum(-1)->maximum(1);
  ...
  RhIO::Root.child("server").newStr("hostname")->defaultValue("testServ")->persisted(true);
  ...
  RhIO::Root.newFrame("test/frame1", "Some frame", RhIO::FrameFormat::RGB);

  RhIO::Root.newCommand("test/command1", "command1", [](const std::vector<std::string>& args) -> std::string {
    if (args.size() != 1)
    {
