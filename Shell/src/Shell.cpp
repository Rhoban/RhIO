    ...
    if ((c = getchar()) > 0)
    {
      switch (c)
      {
        case 0x0a:  // enter
          ...
          if (line.compare("") != 0 && line.compare(prev_cmd) != 0)  // store in history if non null and different than
                                                                     // the last cmd
          {
            shell_history.push_back(line);
        ...
        case 0x12:  // Ctrl-r history completion
          ...
          cur_comp_line = line;
          line = "";
          for (std::deque<std::string>::iterator hist_it = shell_history.begin(); hist_it != shell_history.end();
               ++hist_it)
          {
        ...
        case 0x09:  // TAB completion
          ...
          for (std::vector<std::string>::iterator p_it = paths.begin(); p_it != paths.end(); ++p_it)
          {
            if ((*p_it).compare(0, cur_comp_line.size(), cur_comp_line) == 0)
            {
              ...
              // if((*p_it).find("/",cur_comp_line.size())==std::string::npos)// || cur_comp_line.size()==0 ||
              // cur_comp_line.back()=='/')
              {
